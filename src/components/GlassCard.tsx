        ...
        {(title || Icon) && (
          <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2 flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {title && <h3 className="text-white/90 font-bold tracking-[0.15em] sm:tracking-[0.2em] text-[9px] sm:text-[10px] uppercase bevel-text">{title}</h3>}
            </div>
          </div>
        )}
        
        <div className={`${(title || Icon) ? 'px-4 sm:px-6 pb-4 sm:pb-6 pt-2' : 'p-4 sm:p-6'} h-full relative z-10`}>
          {children}
        </div>
      </div>
    </motion.div>
