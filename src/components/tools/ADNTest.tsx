...
export function ADNTest({ onClose }: { onClose: () => void }) {
  ...
  const handlePlayAudio = () => {
    ...
    if (typeof window !== 'undefined' && window.speechSynthesis) {
      ...
      const textToSpeak = "Please listen carefully: go to the second floor, meet your tutor, and start your class today.";
      ...
      utterance.onstart = () => {
        ...
        setIsPlaying(true);
        setAudioPlays(prev => Math.min(2, prev + 1));
      };
      ...
      utterance.onerror = () => {
        setIsAudioLoading(false);
        setIsPlaying(false);
        // Fallback simulation in case speech synthesis fails or is blocked by iframe constraints
        setIsPlaying(true);
        setAudioPlays(prev => Math.min(2, prev + 1));
        setTimeout(() => {
          setIsPlaying(false);
      ...
      // Fallback for sandboxed environmental setups
      setTimeout(() => {
        setIsAudioLoading(false);
        setIsPlaying(true);
        setAudioPlays(prev => Math.min(2, prev + 1));
      ...
      <div className="max-w-4xl mx-auto w-full p-8 py-20 flex flex-col min-h-screen">
        ...
        <div className="flex items-center justify-between mb-20 text-white/20">
           ...
           <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-white">AI ADN MODULO {step}/3</span>
           </div>
        </div>
                           ...
                           <div className="p-4 md:p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4 relative overflow-hidden">
                              ...
                              {isPlaying && (
                                <motion.div 
                                  ...
                                  animate={{ opacity: 1, y: 0 }}
                                  className="p-3 md:p-4 rounded-xl bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-center space-y-1 mb-4"
                                >
                                   <p className="text-[7px] md:text-[8px] font-black uppercase tracking-wider text-cyan-400/70">CONTENIDO DE PRUEBA / TRANSCRIPT:</p>
                                   <p className="text-[10px] md:text-xs font-semibold italic">"Please listen carefully: go to the second floor, meet your tutor, and start your class today."</p>
                                </motion.div>
                              )}
